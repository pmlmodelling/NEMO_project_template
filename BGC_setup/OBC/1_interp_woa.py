...
with open('1_interp_woa.yaml','r') as yamlfile:
...
grd['mask'] = grd.lon/grd.lon # Set mask to full region to flood area 
...
for v,vconf in yconf['variables'].items():
    ...
    ds_3Dint = xr.Dataset(coords={'time':ds.time,'depth':ds.depth,'lon':grd.lon,'lat':grd.lat})
    ...
    for z in tqdm(range(ds.z.size)):
        ...
        regridder = xesmf.Regridder(ds.isel(t=0,z=z),grd,method='bilinear',extrap_method='nearest_s2d')
