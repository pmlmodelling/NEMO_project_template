MODULE sbcmod
   ...
   USE sbcrnf_ebm      ! surface boundary condition: estuary box model runoffs
...
CONTAINS
   ...
   SUBROUTINE sbc_init
      ...
      NAMELIST/namsbc/ nn_fsbc  ,                                                    &
         ...
         &             ln_tauw  , nn_lsm, nn_sdrift, ln_rnfebm
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
...
!!gm  lk_oasis is controlled by key_oasis3  ===>>>  It shoud be removed from the namelist 
         ...
         WRITE(numout,*) '         estuary box models for runoffs             ln_rnfebm     = ', ln_rnfebm
...
!!gm abusive use of jp_none ??   ===>>> need to be check and changed by adding a jp_sas parameter
      ...
      !
      IF( ln_rnfebm  )    CALL sbc_rnfebm_init         ! estuary box model initialization
      !
   ...
   SUBROUTINE sbc( kt )
      ...
      IF( ln_rnfebm  )    CALL sbc_rnfebm( kt )         ! mix runoffs using estuary box model
